...
export default function StarIcon ({event, selection = false}) {
    // this.state.fav ? localStorage.setItem(`fav${this.props.item}`, `${this.props.item}`) :
    // localStorage.removeItem(`fav${this.props.item}`)
    ...
    return (
      <svg id="star-icon" onClick={event}
      style={selected && {backgroundColor: gold}}
